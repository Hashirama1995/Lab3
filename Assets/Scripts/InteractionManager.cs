...
public class InteractionManager : MonoBehaviour
{
    ...
    private bool TrySelectObject(Vector2 position)
    {
        ...
        if (Physics.Raycast(ray, out hitObject))
        {
            if (hitObject.collider.CompareTag("SpawnedObject"))
            {
                ...
                {
                   if(_selectedObject.GetComponent<SpawnedObjectMoving>().getState() == SpawnedObjectMoving.States.Rotation)
                   {
                        return false;
                   }
                    
                    GameObject var = _selectedObject;
                    StartCoroutine(var.GetComponent<SpawnedObjectMoving>().UnselectedObjectTransform());
                }
                ...
                foreach(GameObject item in GameObject.FindGameObjectsWithTag("SpawnedObject"))
                {
                    if (item != _selectedObject)
                        StartCoroutine(item.GetComponent<SpawnedObjectMoving>().ChangeWaving(true, 1f));
                    else
                        StartCoroutine(item.GetComponent<SpawnedObjectMoving>().SelectedObjectTransform());
                }
    ...
    private bool UnselectObject_changeMoving()
    {
        StartCoroutine(_selectedObject.GetComponent<SpawnedObjectMoving>().UnselectedObjectTransform());

        foreach (GameObject item in GameObject.FindGameObjectsWithTag("SpawnedObject"))
        {
            StartCoroutine(item.GetComponent<SpawnedObjectMoving>().ChangeWaving(false, 1f));
        }
    ...
    private IEnumerator WaitFor(float sec)
    {
        ...
        GameObject secondObj = Instantiate(_spawnedObjectPrehabs[_spawnedObjectType],
                                           _raycastHits[0].pose.position + Vector3.forward * 0.5f,
                                           _spawnedObjectPrehabs[_spawnedObjectType].transform.rotation);
        secondObj.GetComponent<Renderer>().material.color = Color.black;
        yield return new WaitForSeconds(sec);

        GameObject thirdObj = Instantiate(_spawnedObjectPrehabs[_spawnedObjectType],
                                            _raycastHits[0].pose.position + Vector3.right * 0.5f,
                                            _spawnedObjectPrehabs[_spawnedObjectType].transform.rotation);
        thirdObj.GetComponent<Renderer>().material.color = Color.blue; 
    }
